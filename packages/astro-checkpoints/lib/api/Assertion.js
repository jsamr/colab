...
function buildAssertions( assertorDescriptions, methodNameGetter ) {
    ensures('"asserting" argument must be an object.', assertorDescriptions, Object);
    ...
    perform( astroClassInstanceContext, astroMethodParams ) {
        ...
        if( dependingAssertionsOutcome ) return dependingAssertionsOutcome;
            ...
            if( verdict ) {
                ...
                logger.warn( `${cop} ${failChar}  ${className}#${this._methodNameGetter()} : failed assertion '${this.assertor.name}' ( ${reason} )` );
                return buildErrorDescriptor( reason, this.assertor.errorId );
            }
            else {
                logger.info( `${cop}${okChar} ${className}#${this._methodNameGetter()} : passed assertion '${this.assertor.name}' ( ${this.assertor.reason} )` );
                return false;
            }
        }
    }
