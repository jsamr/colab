...
let decorateMock = function( target, propName, decoration ) {
...
describe('astro-checkpoints`', function() {
    ...
    describe('a method decorated with `assert`', function () {
        describe('describing a `DefaultAssertor` instance', function () {
            it('should throw an error of type `SecurityException` when the bound assertion fails ', function () {
                ...
                decorateMock( targetCandidate, 'someMethod', assert( 'alwaysFailingDefaultAssertor', 'assertion argument 1' ));
                expect(function () {
        ...
        describe('describing a `MethodParamsAssertor` instance', function () {
            ...
            decorateMock( targetCandidate, 'someMethod', assert( 'methodArgMustBeAStringAssertor' ));
            it('should throw an error of type `SecurityException` when the bound assertion fails ', function () {
                expect(function () {
    ...
    describe('a method decorated with multiple `assert`s', function () {
        it('should apply all those assertions', function () {
            ...
            // mock the @decorator
            decorateMock( targetCandidate1, 'someMethod', assert( 'alwaysFailingDefaultAssertor', 'assertion argument 1' ));
            decorateMock( targetCandidate1, 'someMethod', assert( 'alwaysPassingDefaultAssertor', 'assertion argument 1' ));
            let targetCandidate2 = {
                someMethod: function () {}
            };
            // mock the @decorator
            decorateMock( targetCandidate2, 'someMethod', assert( 'alwaysPassingDefaultAssertor', 'assertion argument 1' ));
